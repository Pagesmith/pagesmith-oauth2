...
sub fetch_scopes_by_authcode {
  my( $self, $auth_code ) = @_;
  return [ map { $self->make_scope( $_ ) }
         @{ $self->all_hash( 'select '.$self->full_column_names.$self->audit_column_names.'
                                from scope o, authcode_scope acs
                               where o.scope_id = acs.scope_id and acs.authcode_id = ?
                               order by o.scope_id ', $auth_code->uid )||[] } ];
}

sub fetch_scopes_by_accesstoken {
  my( $self, $accesstoken ) = @_;
  return [ map { $self->make_scope( $_ ) }
         @{ $self->all_hash( 'select '.$self->full_column_names.$self->audit_column_names.'
                                from scope o, accesstoken_scope ats
                               where o.scope_id = ats.scope_id and ats.accesstoken_id = ?
                               order by o.scope_id ', $accesstoken->uid )||[] } ];
}

sub fetch_scopes_by_refreshtoken {
  my( $self, $refreshtoken ) = @_;
  return [ map { $self->make_scope( $_ ) }
         @{ $self->all_hash( 'select '.$self->full_column_names.$self->audit_column_names.'
                                from scope o, refreshtoken_scope ats
                               where o.scope_id = ats.scope_id and ats.refreshtoken_id = ?
                               order by o.scope_id ', $refreshtoken->uid )||[] } ];
}
