...
sub fetch_scopes {
   my $self = shift;
   $self->{'scopes'} = {map { $_->uid => $_ } @{$self->get_other_adaptor('Scope')->fetch_scopes_by_authcode( $self )}};
   return $self;
}
...
sub add_scope {
  ...
  unless( exists $self->{'scopes'}{$scope->uid} ) {
...
sub create_accesstoken {
  my $self = shift;
  return $self->get_other_adaptor( 'AccessToken' )->create_from_authcode( $self );
}

sub create_refreshtoken {
  my $self = shift;
  return $self->get_other_adaptor( 'RefreshToken' )->create_from_authcode( $self );
}
