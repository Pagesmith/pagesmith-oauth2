package Pagesmith::Action::OA2::Admin::User;
...
sub run {
#@params (self)
## Display admin for table for User in OA2
  my $self = shift;
  ...
  return $self->my_wrap( q(OA2's User),
    $self
      ->my_table
      ->add_columns(
        { 'key' => 'get_user_id', 'label' => 'User id', 'format' => 'd' },
        { 'key' => 'get_uuid', 'label' => 'Uuid' },
        { 'key' => 'get_method', 'label' => 'Method' },
        { 'key' => 'get_username', 'label' => 'Username' },
        { 'key' => 'get_developer', 'label' => 'Developer' },
        { 'key' => 'get_admin', 'label' => 'Admin' },
        { 'key' => 'get_superadmin', 'label' => 'Superadmin' },
        { 'key' => 'get_projects', 'label' => 'PROJECTS' },
        { 'key' => '_edit', 'label' => 'Edit?', 'template' => 'Edit', 'align' => 'c', 'no_filter' => 1,
          'link' => '/form/OA2_Admin_User/[[h:uid]]' },
      )
      ->add_data( @{$self->adaptor( 'User' )->fetch_all_users||[]} )
      ->render.
    $self->button_links( '/form/OA2_Admin_User', 'Add' ),
  );
  ## use critic
}
