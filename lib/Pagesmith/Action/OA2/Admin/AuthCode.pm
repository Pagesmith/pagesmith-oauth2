package Pagesmith::Action::OA2::Admin::AuthCode;
...
## Admin table display for objects of type AuthCode in
## namespace OA2
...
sub run {
#@params (self)
## Display admin for table for AuthCode in OA2
  my $self = shift;
  ...
  return $self->my_wrap( q(OA2's AuthCode),
    $self
      ->my_table
      ->add_columns(
        { 'key' => 'get_authcode_id', 'label' => 'Authcode id', 'format' => 'd' },
        { 'key' => 'get_uuid', 'label' => 'Uuid' },
        { 'key' => 'get_expires_at', 'label' => 'Expires at' },
        { 'key' => 'get_user_id', 'label' => 'USER' },
        { 'key' => 'get_client_id', 'label' => 'CLIENT' },
        { 'key' => 'get_url_id', 'label' => 'URL' },
        { 'key' => '_edit', 'label' => 'Edit?', 'template' => 'Edit', 'align' => 'c', 'no_filter' => 1,
          'link' => '/form/OA2_Admin_AuthCode/[[h:uid]]' },
      )
      ->add_data( @{$self->adaptor( 'AuthCode' )->fetch_all_authcodes||[]} )
      ->render.
    $self->button_links( '/form/OA2_Admin_AuthCode', 'Add' ),
  );
  ## use critic
}
