...
#@return (string+) - an array of names - these are used in the create_object/update_object code
  ...
  return qw(uuid expires_at user_id client_id refreshtoken_id);
}
...
sub initialize_form {
  ...
  $self->admin_init;
    ...
    my $refreshtokens   = $self->adaptor( 'RefreshToken' )->fetch_all_refreshtokens;
    my $clients         = $self->adaptor( 'Client' )->fetch_all_clients;
    my $users           = $self->adaptor( 'User' )->fetch_all_users;

    ## Unique_ID
    $self->add(           'Hidden',                'accesstoken_id' )->set_optional;
    ...
    $self->add(           'DropDown',              'refreshtoken_id' )
         ->set_firstline(   '== select ==' )
         ->set_caption(     'RefreshToken' )
         ->set_values(      [ map { { 'value' => $_->uid, 'name' => $_->get_refreshtoken_id } } @{$refreshtokens} ] );
