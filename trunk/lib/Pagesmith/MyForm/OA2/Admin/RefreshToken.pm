...
#@return (string+) - an array of names - these are used in the create_object/update_object code
  ...
  return qw(uuid expires_at user_id client_id accesstoken_id);
}
...
sub initialize_form {
  ...
  $self->admin_init;
    ...
    my $accesstokens    = $self->adaptor( 'AccessToken' )->fetch_all_accesstokens;
    my $clients         = $self->adaptor( 'Client' )->fetch_all_clients;
    my $users           = $self->adaptor( 'User' )->fetch_all_users;

    ## Unique_ID
    $self->add(           'Hidden',                'refreshtoken_id' )->set_optional;
    ...
    $self->add(           'DropDown',              'accesstoken_id' )
         ->set_firstline(   '== select ==' )
         ->set_caption(     'AccessToken' )
         ->set_values(      [ map { { 'value' => $_->uid, 'name' => $_->get_accesstoken_id } } @{$accesstokens} ] );
