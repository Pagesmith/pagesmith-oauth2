...
#@return (string+) - an array of names - these are used in the create_object/update_object code
  ...
  return qw(url_type uri project_id);
}
...
sub initialize_form {
  ...
  $self->admin_init;
    ...
    my $url_types       = $self->adaptor( 'Url' )->all_url_type_sorted;

    ## Unique_ID
    $self->add(           'Hidden',                'url_id' )->set_optional;

    $self->add(           'DropDown',              'url_type' )
         ->set_firstline(   '== select ==' )
         ->set_values(      [ map { { 'value' => $_->[0], 'name' => $_->[1] } } @{$url_types} ] );
    $self->add(           'URL',                   'uri' );
