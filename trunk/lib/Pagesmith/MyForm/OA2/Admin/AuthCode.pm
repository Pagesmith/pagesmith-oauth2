...
use Pagesmith::Adaptor::OA2::AuthCode;
use Pagesmith::Object::OA2::AuthCode;

sub fetch_object {
  my $self = shift;##
  my $db_obj        = $self->adaptor( 'AuthCode' )->fetch_auth_code( $self->{'object_id'} );
...
sub create_object {
  ...
  my $new_obj = $self->adaptor( 'AuthCode' )->create;
...
sub initialize_form {
  ...
  $self->set_title( 'AuthCode' )
       ->force_form_code
       ->add_attribute( 'id',     'AuthCode' )
...
## Now add the elements
    ...
    $self->add_stage('AuthCode');
    $self->add_section( 'Administration of AuthCode objects' );
    $self->set_next( 'Create' );

      $self->add('Hidden','authcode_id')
         ->set_optional;
