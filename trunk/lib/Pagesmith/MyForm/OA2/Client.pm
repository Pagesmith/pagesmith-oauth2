...
sub populate_object_values {
  ...
  $self->element( 'type'  )->set_obj_data( $self->object->get_client_type );
...
sub update_object {
  ...
  $self->object->set_client_type(  $self->element( 'type'  )->scalar_value );
  $self->object->store;
  my $current = {};
...
sub create_object {
  ...
  $new_obj->set_client_type(  $self->element( 'type'  )->scalar_value );
...
## Now add the elements
    ...
    $self->set_next( 'Create' );
      ...
      my $types = Pagesmith::Object::OA2::Client->all_client_types_sorted;

      $self->add('DropDown','type')
        ->set_caption( 'Client type' )
        ->set_firstline(   '== select ==' )
        ->set_values(      [ map { { 'value' => $_->[0], 'name' => $_->[1] } } @{$types} ] );
