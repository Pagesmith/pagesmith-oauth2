...
sub run {
  ...
  my $accesstoken = $adap->fetch_accesstoken_by_uuid( $self->param('access_token') );
  ...
  return $self->json_print({
    'email'     => $oa_user->get_email,
    'name'      => $oa_user->get_name,
    'username'  => $oa_user->get_auth_method.q(:).$oa_user->get_username,
    'code'      => $oa_user->get_username,
    'id'        => $oa_user->get_uuid,
  });
}
