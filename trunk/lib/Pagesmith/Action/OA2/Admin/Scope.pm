package Pagesmith::Action::OA2::Admin::Scope;
...
## Admin table display for objects of type Scope in
## namespace OA2
...
sub run {
#@params (self)
## Display admin for table for Scope in OA2
  my $self = shift;
  ...
  return $self->my_wrap( q(OA2's Scope),
    $self
      ->my_table
      ->add_columns(
        { 'key' => 'get_scope_id', 'label' => 'Scope id', 'format' => 'd' },
        ...
        { 'key' => '_edit', 'label' => 'Edit?', 'template' => 'Edit', 'align' => 'c', 'no_filter' => 1,
          'link' => '/form/OA2_Admin_Scope/[[h:uid]]' },
      )
      ->add_data( @{$self->adaptor( 'Scope' )->fetch_all_scopes||[]} )
      ->render.
    $self->button_links( '/form/OA2_Admin_Scope', 'Add' ),
  );
  ## use critic
}
