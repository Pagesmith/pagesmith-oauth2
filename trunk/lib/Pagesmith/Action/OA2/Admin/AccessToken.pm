package Pagesmith::Action::OA2::Admin::AccessToken;
...
## Admin table display for objects of type AccessToken in
## namespace OA2
...
sub run {
#@params (self)
## Display admin for table for AccessToken in OA2
  my $self = shift;
  ...
  return $self->my_wrap( q(OA2's AccessToken),
    $self
      ->my_table
      ->add_columns(
        { 'key' => 'get_accesstoken_id', 'label' => 'Accesstoken id', 'format' => 'd' },
        { 'key' => 'get_uuid', 'label' => 'Uuid' },
        { 'key' => 'get_expires_at', 'label' => 'Expires at' },
        { 'key' => 'get_user_id', 'label' => 'USER' },
        { 'key' => 'get_client_id', 'label' => 'CLIENT' },
        { 'key' => 'get_refreshtoken_id', 'label' => 'REFRESHTOKEN' },
        { 'key' => '_edit', 'label' => 'Edit?', 'template' => 'Edit', 'align' => 'c', 'no_filter' => 1,
          'link' => '/form/OA2_Admin_AccessToken/[[h:uid]]' },
      )
      ->add_data( @{$self->adaptor( 'AccessToken' )->fetch_all_accesstokens||[]} )
      ->render.
    $self->button_links( '/form/OA2_Admin_AccessToken', 'Add' ),
  );
  ## use critic
}
